...
import { HiHeart, HiMiniChatBubbleOvalLeftEllipsis } from "react-icons/hi2";
...
export default function HomePage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/post/getallpost`
      );
  ...
  // New mutations for delete and update comment
          ...
          <div key={post._id} className="flex justify-center mt-6">
            <div className="w-full max-w-md bg-white text-black rounded-lg shadow-md overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-3">
                <Link
                  to={`/profile/${post.user.studentId}`}
